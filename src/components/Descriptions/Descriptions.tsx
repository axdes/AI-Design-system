...
type Props = {
  ...
  /* `columnCount`, not `columns`: four other parts publish `columns` and every
   * one of them means the column DEFINITIONS. This is how many of them there
   * are. (2026-09-03) */
  columnCount?: 1 | 2 | 3
  className?: string
}
...
export function Descriptions({ items, layout = 'stacked', columnCount = 1, className }: Props) {
  return (
    <dl className={cn('descriptions', className)} data-layout={layout} data-column-count={columnCount}>
      {items.map((item, i) => (
