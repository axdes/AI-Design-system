...
type Props = {
  ...
  shape?: Shape
  /** Number of text lines. Only meaningful for shape="text"; the last line is
...
/* Loading placeholder: the shape of content before it arrives, so the layout
...
export function Skeleton({ shape = 'text', lines = 1, width, height, className }: Props) {
    ...
    return (
      <span className={cn('skeleton-lines', className)} aria-hidden="true">
        {Array.from({ length: lines }).map((_, i) => (
          ...
          <span key={i} className="skeleton" data-shape="text" data-last={i === lines - 1 || undefined} />
        ))}
      </span>
    )
  }
