...
describe('Page', () => {
  it('takes its geometry from the archetype', () => {
    ...
    expect(page).toHaveAttribute('data-body', 'single')
    expect(page).toHaveAttribute('data-archetype', 'settings')
  })

  it('lets an explicit prop override the archetype', () => {
    const { container } = render(<Page archetype="settings" width="full" body="board">body</Page>)
    const page = container.querySelector('.page')
    expect(page).toHaveAttribute('data-width', 'full')
    expect(page).toHaveAttribute('data-body', 'board')
  })
  ...
  it('falls back to the single body at the default width with no archetype', () => {
  ...
  it('renders the second pane only in the body that has one', () => {
    const { container, rerender } = render(
      <Page body="list-detail" detail={<p>the selection</p>}>the queue</Page>,
    )
    expect(screen.getByText('the selection')).toBeInTheDocument()
    expect(container.querySelector('.page-detail')).toBeInTheDocument()

    /* Same props, a body with no second pane: the detail is not rendered
       somewhere else, it is not rendered at all. */
    rerender(<Page body="single" detail={<p>the selection</p>}>the queue</Page>)
