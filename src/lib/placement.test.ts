...
const at = (over: Partial<PlacementInput> = {}): PlacementInput => ({
  trigger: { top: 300, bottom: 330, left: 400, right: 500, width: 100 },
...
describe('placeAbove', () => {
  ...
  it('flips up when it does not fit below and there is more room above', () => {
    ...
    expect(placeAbove(at({ trigger: { top: 700, bottom: 730, left: 400, right: 500, width: 100 } }))).toBe(true)
  })
  ...
  it('stays down when it fits nowhere but below is still the roomier side', () => {
    ...
    const input = at({
      ...
      trigger: { top: 100, bottom: 130, left: 400, right: 500, width: 100 },
    })
...
describe('pinsRight', () => {
  ...
  it('flips to the right when a left-aligned menu would run off the right edge', () => {
    const input = at({
      align: 'start',
      trigger: { top: 300, bottom: 330, left: 900, right: 980, width: 80 },
  ...
  it('flips to the left when a right-aligned menu would run off the left edge', () => {
    const input = at({
      align: 'end',
      trigger: { top: 300, bottom: 330, left: 20, right: 100, width: 80 },
      menu: { height: 200, width: 300 },
    })
  ...
  it('does not guess before the menu has been measured', () => {
    ...
    const unmeasured = at({ align: 'start', menu: { height: 0, width: 0 }, trigger: { top: 300, bottom: 330, left: 995, right: 1000, width: 5 } })
...
describe('computePlacement', () => {
  ...
  it('when flipped, sits above the trigger and offers the room above', () => {
    const p = computePlacement(at({ trigger: { top: 700, bottom: 730, left: 400, right: 500, width: 100 } }))
  ...
  it('never places the menu past the top edge', () => {
    ...
    const p = computePlacement(at({
      ...
      trigger: { top: 500, bottom: 530, left: 400, right: 500, width: 100 },
    }))
    expect(p.top).toBe(EDGE_PAD)
  })

  it('keeps the padding when the trigger is against an edge', () => {
    const left = computePlacement(at({ trigger: { top: 300, bottom: 330, left: 0, right: 80, width: 80 } }))
    expect(left.left).toBe(EDGE_PAD)

    const right = computePlacement(at({ align: 'end', trigger: { top: 300, bottom: 330, left: 920, right: 1000, width: 80 } }))
